...
tests:
	docker compose $(COMPOSE_FILE) exec backend poetry run pytest --migrations --create-db --cov $(c)

tests-all:
	docker compose $(COMPOSE_FILE) exec backend poetry run pytest --migrations -p no:warnings --cov-report term:skip-covered --cov --cov-fail-under=93
...
linters:
	docker compose $(COMPOSE_FILES) exec backend poetry run ruff check . --fix --config pyproject.toml

format:
	docker compose $(COMPOSE_FILES) exec backend poetry run ruff format .

lint-check:
	docker compose $(COMPOSE_FILES) exec backend poetry run ruff check . --statistics --config pyproject.toml

# e.g. make chown-migrations c=main_page
chown-migrations:
	sudo chown -R ${USER} ./backend/$(c)/migrations/
