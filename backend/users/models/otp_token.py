...
SMS_MESSAGE_TEMPLATE = "Ваш код для входа в Личный кабинет: %s"
SMS_BOOKING_MESSAGE_TEMPLATE = "Ваш код подтверждения: %s"
SMS_REQUEST_MESSAGE_TEMPLATE = "Ваш код подтверждения: %s"
...
class OTPToken(models.Model):
    ...
    uid = models.UUIDField(verbose_name="UUID", db_index=True, default=uuid4)
    ...
    @classmethod
    def create(cls: type["OTPToken"], phone_number, token_type=OTPTokenType.LOGIN) -> models.Model:
    ...
    @classmethod
    def generate_otp(cls: type["OTPToken"], _: int = 4) -> str:
        """Генерация кода.

        if not settings.TESTING:
            return generate_code(_)
        """
...
class EmailOTPToken(models.Model):
    ...
    @classmethod
    def create(cls: type["EmailOTPToken"], email: str) -> models.Model:
        """Создание одноразового кода."""
    ...
    @classmethod
    def generate_otp(cls: type["EmailOTPToken"], _: int = 4) -> str:
        """Генерация кода.
