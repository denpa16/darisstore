...
class BaseLoggingMixin:
    ...
    def __init__(self, *args, **kwargs) -> None:
        """Инициализация."""
        if not isinstance(self.CLEANED_SUBSTITUTE, str):
            error_msg = "CLEANED_SUBSTITUTE должен быть строкой."
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        if self.should_log(request, response):
            if (
                ...
                connection.set_rollback(1)
                connection.set_rollback(0)
    ...
    def _get_view_name(self, request) -> None | str:
        """Возвращает имя view."""
    ...
    def _get_view_method(self, request) -> str:
...
class LoggingErrorsMixin(LoggingMixin):
    ...
    def should_log(self, request, response) -> bool:
        ...
        return response.status_code >= error_status_code
