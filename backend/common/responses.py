...
class Response(DrfResponse):
    """Кастомный класс drf-ответа."""

    def __init__(self, data=None, status=None, message=None, **kwargs) -> None:
        """Инициализация."""
        status = status if status else self.status_code
        message = message if message else http_status_codes[status].lower()
        if data is None:
            data = {}
        if drf_status.is_success(status):
            ok = True
        elif drf_status.is_client_error(status) or drf_status.is_server_error(status):
            ok = False
        else:
            ok = True
        custom_data = {
        ...
        super().__init__(data=custom_data, status=status, **kwargs)
